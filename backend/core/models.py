...
class Course(models.Model):
    ...
    def recalculate_rating(self):
        """
        Re-derive `rating` from the course's reviews.

        Called whenever a review is written or removed. When the last review
        goes away the existing value is left alone rather than reset, so a
        course that was seeded with a rating does not suddenly drop to zero.
        """
        ...
        self.rating = Decimal(average).quantize(
