...
// (A @blur handler on the trigger button fires *before* the click on a menu
// item registers, which made "My Learning" and "Sign out" unclickable.)
function onDocumentClick(event) {
  if (!menuOpen.value) return
  if (userArea.value && !userArea.value.contains(event.target)) {
...
<template>
  <nav class="navbar">
    ...
    <div class="nav-actions">
      ...
      <!-- Logged in -->
      <div v-else class="user-area" ref="userArea">
        <span class="user-badge">{{ (user?.username || 'U').charAt(0).toUpperCase() }}</span>
        <button
          class="user-menu-btn"
          type="button"
          aria-haspopup="true"
